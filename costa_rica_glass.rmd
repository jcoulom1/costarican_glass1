...
alkali_plot <- ggplot(rock_data_wt, aes(x = SiO2, y = Na2O + K2O)) +
  ...
  scale_color_manual(values = c("coral1", "chartreuse3", "cyan3", "blue2", "deeppink2", "orchid1", "seagreen3", "firebrick3")) +
...
A table comparing mean and standard deviation of Aluminum and Titanium     


```{r table for mean and sd, include=FALSE}
...
cr2a_cr2b_alk <- rock_data_wt %>%
  ...
  scale_color_manual(values = c("cyan3", "blue2")) + 
  guides(color = guide_legend(override.aes = list(size = 5))) +
    theme(text = element_text(size = 15),
          legend.key.size = unit(1.0, "cm"),
          legend.title = element_text(size = 14),
