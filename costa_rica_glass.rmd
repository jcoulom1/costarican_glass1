...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, warn = 2)
...
mg_plot_all <- ggplot(rock_data_wt, aes(x = SiO2, y = MgN)) +
  ...
  scale_shape_manual(values = c(7, 8, 10, 11, 21:25)) + 
  scale_color_manual(values = c("coral1", "chartreuse3", "peachpuff4", "blue2", "deeppink2", "orchid4", "royalblue4", "firebrick3", "cyan3")) +
  guides(color = guide_legend(override.aes = list(size = 5))) +
  theme(text = element_text(size = 15),
          legend.key.size = unit(1.0, "cm"),
          legend.title = element_text(size = 14),
...
glass_avg2 <- as.data.frame(glass_avg)
glass_avg3 <- glass_avg2[,-1]
rownames(glass_avg3) <- glass_avg2[, 1]
glass_avg_tran <- transpose(glass_avg3)
rownames(glass_avg_tran) <- colnames(glass_avg3)
colnames(glass_avg_tran) <- rownames(glass_avg3)
