...
blk_cr2a <- bulk_data %>%
  filter(grepl("CR2A", bulk_data$Comment)) %>%
...
blk_data <- rbind(blk_cr1a, blk_cr1b, blk_cr2a, blk_cr2b, blk_cr3, blk_cr4, blk_cr5, blk_cr6, blk_cr7)
...
cr1_cr2_blkalk <- blk_data_wt %>%
  ...
  ggplot(aes(SiO2, Na2O + K2O, colour = RockName, size = 3)) + 
  geom_point(aes(shape = RockName, color = RockName)) +
...
```{r table2, echo = FALSE, fig.dim=c(10,7)}
bulk_avg <- blk_data_wt %>%
